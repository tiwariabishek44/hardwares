...
class DatabaseHelper {
  ...
  Future<Database> _initDatabase() async {
    ...
    return await openDatabase(
      path,
      version: 4, // Increased version for transactions table
      onCreate: _onCreate,
  ...
  // ===== EXISTING ORDER METHODS (unchanged) =====
  ...
  Future<String> _generateBillCode(Database db) async {
    ...
    const String allChars = numbers + lowercase; // Removed uppercase
    ...
    do {
      ...
      // Ensure we have at least one number and one lowercase letter
      codeChars.add(numbers[random.nextInt(numbers.length)]);
      codeChars.add(lowercase[random.nextInt(lowercase.length)]);

      // Add additional characters from all available chars
      for (int i = 0; i < 2; i++) {
  ...
  Future<int> insertTransaction({
    ...
    required String transactionType, // 'paune_parne' or 'rakam_prapta'
    String? description,
    String? date, // Add this parameter
  }) async {
    final db = await database;
    ...
    try {
      Map<String, dynamic> transaction = {
        ...
        'date': date ??
            DateTime.now()
                .toIso8601String(), // Use provided date or current date
        'created_at': DateTime.now().toIso8601String(),
      };
      ...
      if (party != null) {
        ...
        if (transactionType == 'paune_parne') {
          // Add to balance (amount to receive)
          newBalance = currentBalance + amount;
        } else if (transactionType == 'rakam_prapta') {
          // Subtract from balance (amount received)
          newBalance = currentBalance - amount;
        } else {
    ...
    try {
      ...
      if (transactionData.isNotEmpty) {
        ...
        if (party != null) {
          ...
          if (transactionType == 'paune_parne') {
            // Reverse: subtract from balance
            newBalance = currentBalance - amount;
          } else if (transactionType == 'rakam_prapta') {
            // Reverse: add to balance
            newBalance = currentBalance + amount;
          } else {
            newBalance = currentBalance;
          }
  ...
  // Insert order that comes from Firebase sync (already has bill_code)
  Future<int> insertOrderFromSync(Map<String, dynamic> orderData) async {
    final db = await database;

    try {
      // Check if order with this bill_code already exists
